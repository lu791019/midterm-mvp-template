"""題目 3：音樂串流趨勢分析 — FastAPI"""
from fastapi import FastAPI, HTTPException
import sqlite3, pandas as pd, os
DB_PATH = "pipeline.db"
app = FastAPI(title="音樂串流趨勢分析 API")
def get_conn():
...
def stats():
    conn = get_conn()
    df = pd.read_sql("""SELECT Track, Artist, "Spotify Streams" as streams FROM cleaned_tracks ORDER BY streams DESC LIMIT 20""", conn)
    conn.close()
